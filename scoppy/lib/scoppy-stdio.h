...
#ifndef NDEBUG
#define CTX_DEBUG_PRINT(ctx, fmt, args...) \
    if (ctx->has_stdio) {              \
        printf(fmt, ##args);           \
    } else {                           \
        ctx->debugf(fmt, ##args);      \
...
// For log messages that are OK for the user to see
#define CTX_LOG_PRINT(ctx, fmt, args...) \
    if (ctx->has_stdio) {              \
        printf(fmt, ##args); \
    } else {                           \
        ctx->errorf(fmt, ##args);      \
...
#define CTX_ERROR_PRINT(ctx, fmt, args...) \
    if (ctx->has_stdio) {              \
        printf("Error: "); printf(fmt, ##args); \
    } else {                           \
        ctx->errorf(fmt, ##args);      \
    }
